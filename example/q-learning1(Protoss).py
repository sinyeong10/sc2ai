...
# 프로토스 종족의 유닛 및 건물 유형의 ID를 정의
# 프로토스 종족의 특정 유닛 또는 건물을 나타내는 ID를 참조
_PROTOSS_NEXUS = 59
_PROTOSS_PYLON = 60
_PROTOSS_GATEWAY = 62
...
ACTION_SELECT_STARGATE = 'selectstargate'
ACTION_BUILD_VOIDRAY = 'buildvoidray'
...
smart_actions = [
    ...
    ACTION_SELECT_STARGATE,
    ACTION_BUILD_VOIDRAY,
    ACTION_SELECT_ARMY,
    ACTION_ATTACK,
]
...
def step(self, obs):
    ...
    # PROBE 선택
    # 이 분기는 smart_action이 ACTION_SELECT_PROBE일 때 실행. 이는 게임 내에서 PROBE 유닛을 선택하려는 액션을 나타냄.
    # 첫 번째로, 현재 화면에서의 모든 유닛의 유형을 unit_type 변수에 저장.
    # 두 번째로, 해당 유닛 유형 중에서 프로토스의 PROBE 유닛(_PROTOSS_PROBE)의 위치를 찾음. .nonzero() 메소드는 PROBE 유닛의 y좌표와 x좌표를 반환
    elif smart_action == ACTION_SELECT_PROBE:
        unit_type = obs.observation['screen'][_UNIT_TYPE]
        unit_y, unit_x = (unit_type == _PROTOSS_PROBE).nonzero()

        # 만약 PROBE 유닛이 화면에 존재한다면 (unit_y.any()), PROBE의 y좌표 중에서 무작위로 하나의 y좌표를 선택
        # 그 후, 선택된 y좌표에 해당하는 x좌표와 함께 target 변수에 PROBE의 위치를 저장
    ...
    # smart_action이 ACTION_BUILD_PYLON (즉, 공급 저장소를 건설하는 액션)일 경우 실행되는 로직
        ...
        if _BUILD_PYLON in obs.observation['available_actions']:
            ...
            # Implement building pylon logic...
            unit_type = obs.observation['screen'][_UNIT_TYPE]
    ...
    # smart_action이 ACTION_BUILD_STARGATE일 경우 실행되는 로직
    elif smart_action == ACTION_BUILD_STARGATE:
        # STARGATE 건설 가능성 확인
        # 현재 가능한 액션들 중에서 STARGATE 건설 액션이 포함되어 있는지 확인
        # 만약 건설이 가능하지 않다면 (예: 자원 부족, 기타 조건 미충족 등), 이 분기는 실행되지 않음
        if _BUILD_STARGATE in obs.observation['available_actions']:
            # 테란 커맨드 센터 위치 확인
            # BARRACKS을 건설하기 위해 테란 커맨드 센터 (즉, 기본 건물)의 위치를 확인합니다.
            # unit_type 변수에 현재 화면의 모든 유닛 유형을 저장하고, 이 중에서 테란 커맨드 센터의 위치를 unit_y와 unit_x에 저장
            # Implement building stargate logic...
            unit_type = obs.observation['screen'][_UNIT_TYPE]
            unit_y, unit_x = (unit_type == _TERRAN_COMMANDCENTER).nonzero()
    ...
    # smart_action이 ACTION_SELECT_STARGATE일 경우 실행되는 로직. STARGATE을 선택하는 동작을 수행
    elif smart_action == ACTION_SELECT_STARGATE:
        # STARGATE 위치 확인
        # 화면에 표시된 모든 유닛의 유형을 unit_type 변수에 저장
        # unit_y와 unit_x 변수에는 _PROTOSS_STARGATE의 y좌표와 x좌표가 각각 저장
        unit_type = obs.observation['screen'][_UNIT_TYPE]
        unit_y, unit_x = (unit_type == _PROTOSS_STARGATE).nonzero()

        # STARGATE 선택 위치 결정
        # 화면 내에 STARGATE이 존재하는 경우 (unit_y.any()가 참인 경우), STARGATE을 선택하기 위한 목표 위치(target)를 결정
        # 여기서는 화면 내에 존재하는 모든 병영의 평균 x 좌표와 y 좌표를 사용하여 선택할 STARGATE의 대략적인 위치를 결정
        if unit_y.any():
            target = [int(unit_x.mean()), int(unit_y.mean())]
            # STARGATE 선택 액션 반환
            # 결정된 위치 (target)를 사용하여 STARGATE 선택 액션을 반환. _NOT_QUEUED는 해당 동작이 다른 동작들에 추가되지 않고 바로 실행되도록 함
            return actions.FunctionCall(_SELECT_POINT, [_NOT_QUEUED, target])
    # 코드의 목적은 화면 내에 존재하는 STARGATE 중 하나를 선택하는 것. 평균 좌표를 사용하는 것은 화면 내에 여러 개의 STARGATE이 있을 경우, 중앙에 위치한 STARGATE을 대상으로 하기 위함

    # smart_action 값에 따라 다른 행동을 수행
    # VOIDRAY 생성
    # smart_action이 VOIDRAY 생성을 나타내는 ACTION_BUILD_VOIDRAY이면서, 해당 행동이 가능한 액션 중 하나인 경우 (_TRAIN_VOIDRAY in obs.observation['available_actions']), STARGATE에서 VOIDRAY을 생산하는 명령을 반환
    # 여기서 _QUEUED는 해당 동작을 다른 동작들 뒤에 추가하여 순차적으로 실행되도록 합
    elif smart_action == ACTION_BUILD_VOIDRAY:
        if _TRAIN_VOIDRAY in obs.observation['available_actions']:
            return actions.FunctionCall(_TRAIN_VOIDRAY, [_QUEUED])
