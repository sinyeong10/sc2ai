...
class IncrediBot(BotAI):
    async def on_step(self, iteration: int):
        ...
        if self.townhalls:  # do we have a nexus?
            ...
            if self.structures(UnitTypeId.NEXUS).amount <= 1 and not self.already_pending(UnitTypeId.NEXUS):
            ...
            if nexus.is_idle and self.can_afford(UnitTypeId.PROBE) and supply_remaining > 2 and self.units(UnitTypeId.PROBE).amount\
                +self.structures(UnitTypeId.ASSIMILATOR).amount < 3*(self.structures(UnitTypeId.ASSIMILATOR).amount)\
                    +2*len(self.mineral_field.closer_than(10, self.townhalls.first))*self.structures(UnitTypeId.NEXUS).amount:
            ...
            elif self.structures(UnitTypeId.ASSIMILATOR).amount <= 3 and self.can_afford(UnitTypeId.ASSIMILATOR) and not self.already_pending(UnitTypeId.ASSIMILATOR):
            ...
            if self.structures(UnitTypeId.STARGATE).amount <= 2 and self.can_afford(UnitTypeId.STARGATE) and self.already_pending(UnitTypeId.STARGATE) <= 2:
